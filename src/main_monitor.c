/*
 * ==============================================================================
 * 파일명: main_monitor.c
 * 역할: [P4] 사용자 모니터링 및 설정 프로세스
 *
 * 기술 요소:
 *   - Shared Memory: 임계값 설정 읽기/쓰기, 제어 상태 읽기
 *   - Semaphore: Race Condition 방지를 위한 동기화
 *   - select(): 논블로킹 입력으로 종료 신호 감지
 *   - CLI 메뉴 인터페이스
 *
 * 작성자: Virtual SmartFarm Team
 * 작성일: 2025-12-02
 * ==============================================================================
 */

#include "../include/common.h"
#include <sys/select.h>
#include <sys/utsname.h>

static int shm_id = -1;
static int sem_id = -1;
static SharedData *shared_data = NULL;

/* ============================================================================
 * 함수: display_system_info
 * 설명: 시스템 및 프로세스 정보 출력 (uname, getpid 사용)
 * ============================================================================ */
void display_system_info() {
    struct utsname sys_info;
    
    printf("\n");
    printf("┌─────────────────────────────────────────┐\n");
    printf("│          🖥️  시스템 정보               │\n");
    printf("├─────────────────────────────────────────┤\n");
    
    if (uname(&sys_info) == 0) {
        printf("│  OS 이름:     %-24s │\n", sys_info.sysname);
        printf("│  OS 버전:     %-24s │\n", sys_info.release);
        printf("│  호스트명:    %-24s │\n", sys_info.nodename);
        printf("│  아키텍처:    %-24s │\n", sys_info.machine);
    }
    
    printf("├─────────────────────────────────────────┤\n");
    printf("│          📊 프로세스 정보              │\n");
    printf("├─────────────────────────────────────────┤\n");
    printf("│  모니터 PID:  %-24d │\n", getpid());
    printf("│  부모 PID:    %-24d │\n", getppid());
    printf("│  사용자 ID:   %-24d │\n", getuid());
    printf("│  그룹 ID:     %-24d │\n", getgid());
    printf("└─────────────────────────────────────────┘\n");
}

/* ============================================================================
 * 함수: cleanup_and_exit
 * 설명: 시그널 핸들러 - 프로세스 종료 시 자원 정리
 * ============================================================================ */
void cleanup_and_exit(int signo) {
    (void)signo;  // unused parameter 경고 방지
    printf("\n[MONITOR] 종료 중...\n");
    if (shared_data != NULL) {
        shmdt(shared_data);
    }
    exit(0);
}

/* ============================================================================
 * 함수: check_system_running
 * 설명: 서버 종료 신호 확인
 * 반환: 1=실행중, 0=종료됨
 * ============================================================================ */
int check_system_running() {
    sem_lock(sem_id);
    int running = shared_data->system_running;
    sem_unlock(sem_id);
    return running;
}

/* ============================================================================
 * 함수: display_menu
 * 설명: CLI 메뉴 출력
 * ============================================================================ */
void display_menu() {
    printf("\n");
    printf("╔════════════════════════════════════════════════╗\n");
    printf("║   🎛️  SMART FARM CONFIGURATION MENU 🎛️       ║\n");
    printf("╠════════════════════════════════════════════════╣\n");
    printf("║  1. 온도 임계값 설정                          ║\n");
    printf("║  2. 습도 임계값 설정                          ║\n");
    printf("║  3. 현재 설정 및 상태 확인                    ║\n");
    printf("║  4. 시스템 정보 확인                          ║\n");
    printf("║  5. 종료                                      ║\n");
    printf("╚════════════════════════════════════════════════╝\n");
    printf("선택: ");
    fflush(stdout);
}

/* ============================================================================
 * 함수: display_status
 * 설명: 현재 설정값과 제어 상태 출력
 * ============================================================================ */
void display_status() {
    sem_lock(sem_id);
    printf("\n");
    printf("┌─────────────────────────────────────────┐\n");
    printf("│          📊 현재 시스템 상태            │\n");
    printf("├─────────────────────────────────────────┤\n");
    printf("│  [현재 환경]                            │\n");
    printf("│    🌡️  온도: %6.1f°C                   │\n", shared_data->current_temp);
    printf("│    💧 습도: %6.1f%%                    │\n", shared_data->current_humidity);
    printf("├─────────────────────────────────────────┤\n");
    printf("│  [임계값 설정]                          │\n");
    printf("│    온도 임계값: %3d°C                   │\n", shared_data->temp_threshold);
    printf("│    습도 임계값: %3d%%                    │\n", shared_data->humidity_threshold);
    printf("├─────────────────────────────────────────┤\n");
    printf("│  [현재 제어 상태]                       │\n");
    printf("│    🔥 히터: %s                         │\n", shared_data->heater_on ? "ON " : "OFF");
    printf("│    💨 팬:   %s                         │\n", shared_data->fan_on ? "ON " : "OFF");
    printf("│    💡 LED:  %s                         │\n", shared_data->led_on ? "ON " : "OFF");
    printf("└─────────────────────────────────────────┘\n");
    sem_unlock(sem_id);
}

/* ============================================================================
 * 함수: input_available
 * 설명: select()를 사용하여 입력이 있는지 확인 (타임아웃: 1초)
 * 반환: 1=입력 있음, 0=입력 없음
 * ============================================================================ */
int input_available() {
    fd_set fds;
    struct timeval tv;
    
    FD_ZERO(&fds);
    FD_SET(STDIN_FILENO, &fds);
    
    tv.tv_sec = 1;   // 1초 타임아웃
    tv.tv_usec = 0;
    
    return select(STDIN_FILENO + 1, &fds, NULL, NULL, &tv) > 0;
}

/* ============================================================================
 * 메인 함수
 * ============================================================================ */
int main() {
    printf("==================================================\n");
    printf("  가상 스마트팜 모니터 프로세스 [P4] 시작\n");
    printf("==================================================\n");
    printf("  PID: %d, 부모 PID: %d\n\n", getpid(), getppid());

    // 시그널 핸들러 등록
    signal(SIGINT, cleanup_and_exit);
    signal(SIGTERM, cleanup_and_exit);

    // ========================================================================
    // IPC 자원 연결
    // ========================================================================

    // 공유 메모리 연결
    shm_id = shmget(SHM_KEY, sizeof(SharedData), 0666);
    if (shm_id == -1) {
        perror("[MONITOR] 공유 메모리 연결 실패 (서버를 먼저 실행하세요)");
        exit(1);
    }
    shared_data = (SharedData *)shmat(shm_id, NULL, 0);
    if (shared_data == (void *)-1) {
        perror("[MONITOR] 공유 메모리 attach 실패");
        exit(1);
    }
    printf("[MONITOR] 공유 메모리 연결 성공 (ID: %d)\n", shm_id);

    // 세마포어 연결
    sem_id = semget(SEM_KEY, 1, 0666);
    if (sem_id == -1) {
        perror("[MONITOR] 세마포어 연결 실패 (서버를 먼저 실행하세요)");
        exit(1);
    }
    printf("[MONITOR] 세마포어 연결 성공 (ID: %d)\n", sem_id);

    // ========================================================================
    // 메인 루프: CLI 메뉴 (select 기반 논블로킹)
    // ========================================================================
    int choice;
    int menu_displayed = 0;
    
    while (1) {
        // 시스템 종료 확인
        if (!check_system_running()) {
            printf("\n[MONITOR] 서버 종료 신호 수신. 프로세스 종료.\n");
            break;
        }

        // 메뉴 표시 (한 번만)
        if (!menu_displayed) {
            display_menu();
            menu_displayed = 1;
        }

        // 입력 대기 (1초 타임아웃) - 타임아웃 후 종료 신호 다시 체크
        if (!input_available()) {
            continue;
        }

        // 입력 처리
        if (scanf("%d", &choice) != 1) {
            while (getchar() != '\n');
            printf("❌ 숫자를 입력해주세요.\n");
            menu_displayed = 0;
            continue;
        }

        menu_displayed = 0;  // 다음 루프에서 메뉴 다시 표시

        switch (choice) {
            case 1: {
                int new_temp;
                printf("새로운 온도 임계값 (20~40°C): ");
                if (scanf("%d", &new_temp) == 1 && new_temp >= 20 && new_temp <= 40) {
                    sem_lock(sem_id);
                    shared_data->temp_threshold = new_temp;
                    sem_unlock(sem_id);
                    printf("✅ 온도 임계값이 %d°C로 설정되었습니다.\n", new_temp);
                } else {
                    printf("❌ 유효하지 않은 값입니다. (20~40 범위)\n");
                    while (getchar() != '\n');
                }
                break;
            }
            case 2: {
                int new_hum;
                printf("새로운 습도 임계값 (30~90%%): ");
                if (scanf("%d", &new_hum) == 1 && new_hum >= 30 && new_hum <= 90) {
                    sem_lock(sem_id);
                    shared_data->humidity_threshold = new_hum;
                    sem_unlock(sem_id);
                    printf("✅ 습도 임계값이 %d%%로 설정되었습니다.\n", new_hum);
                } else {
                    printf("❌ 유효하지 않은 값입니다. (30~90 범위)\n");
                    while (getchar() != '\n');
                }
                break;
            }
            case 3:
                display_status();
                break;
            case 4:
                display_system_info();
                break;
            case 5:
                cleanup_and_exit(0);
                break;
            default:
                printf("❌ 잘못된 선택입니다. (1~5)\n");
        }
    }

    cleanup_and_exit(0);
    return 0;
}
